...
struct base_ID3D10Texture3D
    : ID3D10Texture3D
{
    explicit base_ID3D10Texture3D(ID3D10Texture3D *impl);
    virtual ~base_ID3D10Texture3D() {}

    HRESULT STDMETHODCALLTYPE Map(UINT Subresource, D3D10_MAP MapType, UINT MapFlags, D3D10_MAPPED_TEXTURE3D* pMappedTex3D) override;
    void STDMETHODCALLTYPE Unmap(UINT Subresource) override;
    void STDMETHODCALLTYPE GetDesc(D3D10_TEXTURE3D_DESC* pDesc) override;
...
public:
    ID3D10Texture3D *impl() const;

private:
    ID3D10Texture3D *impl_;
